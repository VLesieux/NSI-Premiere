...
1) Réalisez l'architecture d'un réseau en plaçant deux ordinateurs Notebook ; donnez-leur des noms simples M1 et M2 et des adresses IP simples : 192.168.0.1 et  192.168.0.2 ; les relier par un câble Ethernet puis passer en mode simulation (triangle vert). En cliquant sur une machine, on ouvre une fenêtre qui permet d'utiliser un logiciel sur un ordinateur ; on utilise le logiciel Command Line pour écrire des **lignes de commande** : **ipconfig** pour vérifier l'IP de la machine, puis on envoie un **ping** sur la  machine M2 en renseignant son adresse IP. Les échanges de packets sont visibles par le changement de couleur du câble réseau. Revenonons maintenant à la configuration du réseau et supprimons le câble (clic droit) pour placer un troisième ordinateur M3 d'IP 192.168.0.3 ; connectons les trois ordinateurs à un switch ; faire maintenant un ping depuis M1 vers M3 en repassant au mode simulation pour vérifier la transmission et la réception des paquets par l'intermédiaire du Switch.

2) Réaliser deux réseaux comportant chacun trois machines (M1,M2,M3) et (M4,M5,M6) et deux Switch R1 et R2 ; l'adresse réseau de R1 est 192.168.1.0 et celle de R2 est 192.168.2.0. Relier les deux réseaux grâce à un Routeur. En cliquant sur le Routeur, renseigner l'adresse IP 192.168.1.254 pour le réseau R1 et 192.168.2.254  pour le réseau R2. Sélectionner dans l'onglet Général 'Routage automatique' pour que le routeur gère automatiquement les tables de routage ; puis pour chaque machine du réseau R1 et pour chaque machine du réseau R2, renseigner le Gateway ou Passerelle correspondant à l'adresse IP du routeur. Testons l'architecture en faisant un ipconfig sur une machine pour voir l'IP du routeur puis en réalisant un ping entre deux machines n'appartenant pas au même réseau; utiliser la commande **traceroute** pour suivre le chemin suivi par un paquet de données.

3) Ajouter maintenant un ordinateur Computeur que l'on nomme "Serveur DNS" en le câblant au routeur après avoir ajouté une connexion dans Gérer les connexions ; renseigner l'IP 192.168.3.1 et le Gateway 192.168.3.254 ; puis en cliquant sur le Router après connexion ajouter l'adresse IP 192.168.3.254. Puis en cliquant sur la machine M1 changer Domain Name Serveur ou Serveur DNS en lui donnant l'adresse IP 192.168.3.1. Passer en mode simulation, cliquer sur l'ordinateur Serveur DNS et installer le logiciel DNS server ; ouvrir le logiciel, donner le nom de domaine M4 et l'adresse IP 192.168.2.1, ajouter puis démarrer. À partir de M1 maintenant, on peut écrire directement ' ping M4 ' car le serveur DNS a associé le nom M4 à l'adresse IP de la machine.

4) Ajouter maintenant un ordinateur Computeur que l'on nomme "Serveur Web" à un réseau constitué de deux ordinateurs M1 et M2 en connectant ces trois machines à l'aide d'un Switch ; en mode simulation cliquer sur la machine Serveur Web pour installer les logiciels File explorer, Texteeditor et Webserveur ; créer une page web simple avec l'éditeur ; à partir de la machine M1 , installer un Webbrowser et visualiser la page en écrivant l'url correspondant à l'IP du Serveur Web : http://192.168.1.3/ c'est la page index.html qui est visualisée que l'on peut modifier avec l'éditeur de texte.

5) Ajouter maintenant un serveur DNS à la configuration précédente pour pouvoir visualiser la page http://my_web depuis l'ordinateur M1.