...
def select_competitor_by_birth_year(dictionnaire,annee_naissance):
    liste=[]
    for cle, valeur in dictionnaire.items():
        if valeur['birth_date'].endswith(str(annee_naissance)):
            liste.append(valeur)
    return liste
...
def sort_competitors_by_lastname(dictionnaire):
    new={}
    liste=[]
    for cle, valeur in dictionnaire.items():
        liste.append(valeur['last_name'])
    liste=Tri.tri_selection(liste,Tri.compare_chaine_lexicographique)
    for i in range(len(liste)):
        for cle, valeur in dictionnaire.items():
            if liste[i]==valeur['last_name']:
