...
def indice_plus_proche(ville, liste_ville, table):
    """
    Renvoie l'indice de la ville la plus proche de ville
...
def indice_plus_proche_methode2(ville, liste_ville, table):
    """
    Renvoie l'indice de la ville la plus proche de ville
    >>> indice_plus_proche_methode2(10,liste_ville("exemple.txt"),matrice_distances('exemple.txt'))
    9
    """
    liste=[table[ville][j] for j in range(len(liste_ville))]
...
def glouton(ind_ville,liste_ville,mat_dist):
    ...
    while len(tour)!=l:
        ...
        liste_ville.remove(ind_ville)#on retire la ville par laquelle on est passé
