...
De manière similaire, faites le travail nécessaire pour définir une fonction `sort_competitors_by_performance` qui produit la liste des compétiteurs triée par ordre croissant des performances réalisées. On utilisera ici la fonction `Tri.compare_entier_croissant`.
...
On utilise pour cela un algorithme de tri appelé **tri par sélection** décrit dans le module Tri et reformulé à nouveau ci-dessous (il sera revu plus tard lors de l'étude des algorithmes de tri):

On recherche l'indice du minimum de la liste et non la valeur du minimum de la liste pour réaliser ensuite la permutation des valeurs de la liste grâce à leur indice selon :
...
def new_select_competitor_by_birth_year(dic,comp):
    ...
    """
```
**Remarque** : on sera amené à réaliser une petite modification de la fonction `selection_min(dic,i,comp)` pour tenir compte de tous les compétiteurs. À cet effet, la docstring de cette fonction sera mise en commentaire.
