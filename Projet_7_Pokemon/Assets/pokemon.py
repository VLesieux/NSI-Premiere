...
POKE_PROP_USED_FOR_DISTANCE = [
    'base_egg_steps', 'capture_rate', 'experience_grow', 'generation'
    ]
...
def pokemon_euclidian_distance(poke1, poke2, min_max_val):
    ...
    def normalized_diff(p1, p2, key):

        (min_val, max_val) = min_max_val[key]

        if max_val == min_val:
            return 0

        # Solution avec getattr :
        # return (getattr(p1, key) - getattr(p2, key)) / (max_val - min_val)

        # solution avec _asdict :
        p1 = p1._asdict()
        p2 = p2._asdict()

        return (p1[key] - p2[key]) / (max_val - min_val)

    return sqrt(sum(
        (normalized_diff(poke1, poke2, key)) ** 2
        for key in POKE_PROP_USED_FOR_DISTANCE
        ))
