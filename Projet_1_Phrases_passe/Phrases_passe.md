...
On se référera à l'algorithme déjà étudié pour la conversion en binaire (en base 2) d'un nombre décimal (cf. la fonction `conversion_decimal_binaire(dec)`), puis on écrira une fonction `lancer_de_des(dec)` avec sa docstring.
